...
__global__ auto advect_velocity_kernel(GridView<float2> velocity_next, GridView<float2> velocity) -> void {
  ...
  if (i <= velocity.width && j <= velocity.height) {
    ...
    const auto velocity_u {bilerp(velocity.ro(i_u - 1, j_u - 1).x, velocity.ro(i_u - 1, j_u).x, velocity.ro(i_u, j_u - 1).x, velocity.ro(i_u, j_u).x,
    ...
    const auto velocity_v {bilerp(velocity.ro(i_v - 1, j_v - 1).y, velocity.ro(i_v - 1, j_v).y, velocity.ro(i_v, j_v - 1).y, velocity.ro(i_v, j_v).y,
