#include "simulation/kernels/external.cuh"

#include "simulation/kernels/parameters.cuh"
#include "utils/cuda.hpp"

namespace fluidsim::simulation::kernels {

namespace {

__global__ auto add_external_force_kernel(GridView<float2> velocity, f32 x, f32 y, f32 f_x, f32 f_y) -> void {
  const auto i {threadIdx.x + blockIdx.x * blockDim.x + 1};
  const auto j {threadIdx.y + blockIdx.y * blockDim.y + 1};

  // This kernel 'incorrectly' sets some boundary values for the velocity.
  // However, they get corrected when updating the boundary values.
  if (i <= velocity.width && j <= velocity.height) {
    const auto dx {i - x};
    const auto dy {j - y};
    const auto r2 {dx * dx + dy * dy};
    const auto radius {external_force_radius};

    if (r2 < radius * radius) {
      const auto linear_falloff {1.0f - (sqrtf(r2) / radius)};
      const auto vel {velocity.at(i, j)};

      velocity.at(i, j) = make_float2(vel.x + dt * density_inverse * linear_falloff * f_x, //
                                      vel.y + dt * density_inverse * linear_falloff * f_y);
    }
  }
}

__global__ auto add_external_dye_kernel(GridView<float4> dye, f32 x, f32 y, f32 r, f32 g, f32 b) -> void {
  const auto i {threadIdx.x + blockIdx.x * blockDim.x + 1};
  const auto j {threadIdx.y + blockIdx.y * blockDim.y + 1};

  if (i <= dye.width && j <= dye.height) {
    const auto dx {i - x};
    const auto dy {j - y};
    const auto r2 {dx * dx + dy * dy};
    const auto radius {external_dye_radius};

    if (r2 < radius * radius) {
      const auto linear_falloff {1.0f - (sqrtf(r2) / radius)};
      const auto d {dye.at(i, j)};

      dye.at(i, j) = make_float4(fminf(1.0f, d.x + dt * linear_falloff * r), //
                                 fminf(1.0f, d.y + dt * linear_falloff * g), //
                                 fminf(1.0f, d.z + dt * linear_falloff * b), //
                                 1.0f);
    }
  }
}

} // namespace

auto add_external_force(GridView<float2> velocity, f32 x, f32 y, f32 f_x, f32 f_y, u32 block_size) -> void {
  const auto [blocks, threads] {utils::execution_configuration(velocity.width, velocity.height, block_size)};
  add_external_force_kernel<<<blocks, threads>>>(velocity, x, y, f_x, f_y);
  utils::check_async_cuda_error();
}

auto add_external_dye(GridView<float4> dye, f32 x, f32 y, f32 r, f32 g, f32 b, u32 block_size) -> void {
  const auto [blocks, threads] {utils::execution_configuration(dye.width, dye.height, block_size)};
  add_external_dye_kernel<<<blocks, threads>>>(dye, x, y, r, g, b);
  utils::check_async_cuda_error();
}

} // namespace fluidsim::simulation::kernels
